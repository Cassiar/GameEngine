...
Mesh::Mesh(const char* path, const char* texpath, Microsoft::WRL::ComPtr<ID3D11Device> device, Microsoft::WRL::ComPtr<ID3D11DeviceContext> context) {
	...
	for (unsigned int i = 0; i < model->GetVertexCount(); i++) {
		Vertex vert = {};
		//slight translation error here :)
		vert.Position = XMFLOAT3(model->GetPositions()[i][0], model->GetPositions()[i][1], model->GetPositions()[i][2]);
		vert.Normal = XMFLOAT3(model->GetNormals()[i][0], model->GetNormals()[i][1], model->GetNormals()[i][2]);
		vert.UVCoord = XMFLOAT2(model->GetUVs()[i][0], model->GetUVs()[i][1]);
